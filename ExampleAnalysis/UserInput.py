...
UserChoices['applyReferenceMassFragmentsThresholds']['referenceMassFragmentFilterThreshold'] = [1.01]
UserChoices['applyReferenceMassFragmentsThresholds']['referenceSignificantFragmentThresholds'] = [20.0]
...
UserChoices['uncertainties']['referencePatterns_uncertainties'] = 'File' #which can be a float/integer for absolute uncertainties or the value True (or the value 'File'. Will expect same file name as reference file with _uncertainties after that) or the value None (False will also be set to None) . For example, the value 2 would mean a 2% uncertainty for the value 100, but a 50% uncertainty for the value of 4.
UserChoices['uncertainties']['dataToAnalyze_uncertainties'] =  2 # Can be 'Auto' or 'File' or 'None' or an Integer like 3 (no quotation marks). If 'File', will expect same file name as collected file with _uncertainties after that). An integer defines a point radius. 'Auto' simply uses a point radius of 5.
