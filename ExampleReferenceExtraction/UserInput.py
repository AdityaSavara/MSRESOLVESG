...
UserChoices['inputFiles']['referencePatternsFileNamesList'] = ['ExistingReferencePatterns.csv'] #enter the file name of the file containing reference information
...
UserChoices['inputFiles']['dataToAnalyzeFileName'] = 'Collected_Reference.csv'	#enter the file name with raw mass spectrometer data
...
UserChoices['linearBaselineCorrectionSemiAutomatic']['massesToBackgroundCorrect'] = []			#mflist: enter mass list delimited With commas [m1, m2, m3]
# to apply a uniform time range to all fragments, only insert one time range as such [[x,y]]
UserChoices['linearBaselineCorrectionSemiAutomatic']['earlyBaselineTimes'] = [[3425,3445]]	# to apply different times for each fragment enter time pairs as such [[x,y],[z,w]..]
# to apply a uniform time range to all fragments, only insert one time range [[x,y]]
UserChoices['linearBaselineCorrectionSemiAutomatic']['lateBaselineTimes'] = [[3700,3720]]	#if you do not wish to enter a second time list enter 0's [[0,0],[0,0]...] or [[0,0]]
...
UserChoices['extractReferencePatternFromDataOption']['on'] = 'yes'
UserChoices['extractReferencePatternFromDataOption']['rpcMoleculesToChange'] = ['Ethylene']
...
UserChoices['extractReferencePatternFromDataOption']['rpcTimeRanges'] = [
                                                                         [3485,3505], #For each molecule to be changed, a pair of times is required.
                                                                         ]
...
UserChoices['extractReferencePatternFromDataOption']['rpcMoleculesToChangeMF'] = [
                                                                                  [27], #For each molecule for using the rpc on, make a new line with a list of masses (length of each should be greater than 1).
                                                                                  ]
...
UserChoices['uncertainties']['dataToAnalyze_uncertainties'] = 'Auto' # or 'File' or 'None'. Will expect same file name as collected file with _uncertainties after that).
