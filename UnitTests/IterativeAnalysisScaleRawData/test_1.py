...
if os.path.isfile('TotalConcentrationsIterative.csv'):
    os.remove('TotalConcentrationsIterative.csv')
...
#ITERATION 3
listOfStringsToAppend = ["chosenMoleculesNames = ['H2']", "chosenMassFragments = [2]"]
...
#ITERATION 7
#now ready for iteration three.
listOfStringsToAppend = ["chosenMoleculesNames = ['CO', 'CO2','Ethylene (Ethene)']", "chosenMassFragments = [27, 28, 44]"]
...
arrayReadFromIterativeAnalysis = np.genfromtxt("TotalConcentrationsIterative.csv", skip_header = 1, delimiter=",", unpack=True)
...
arrayToFillFromIterativeAnalysis[1] = arrayReadFromIterativeAnalysis[6]
arrayToFillFromIterativeAnalysis[2] = arrayReadFromIterativeAnalysis[4]
...
arrayToFillFromIterativeAnalysis[5] = arrayReadFromIterativeAnalysis[9]
arrayToFillFromIterativeAnalysis[6] = arrayReadFromIterativeAnalysis[5]
arrayToFillFromIterativeAnalysis[7] = arrayReadFromIterativeAnalysis[1]
arrayToFillFromIterativeAnalysis[8] = arrayReadFromIterativeAnalysis[3]
arrayToFillFromIterativeAnalysis[9] = arrayReadFromIterativeAnalysis[2]
...
expectedResult = (roundedArrayReadFromNonIterativeAnalysis)# Can't use simulated case for iterative with scaling. -->#,roundedSimulatedArrayReadFromNonIterative) #Make the expected results a tuple

ut.set_expected_result(expectedResult,expected_result_str=str(expectedResult), prefix=prefix,suffix=suffix)

resultObj = (roundedArrayFilledFromIterativeAnalysis)# Can't use simulated case for iterative with scaling. -->#,roundedSimulatedArrayReadFromIterativeAnalysis)
