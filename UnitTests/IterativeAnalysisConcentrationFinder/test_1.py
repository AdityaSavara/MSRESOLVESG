...
if os.path.isfile('ResolvedConcentrations.csv'): #check if file is there
    os.remove('ResolvedConcentrations.csv') #remove it if present
...
arrayReadFromNonIterativeAnalysis = np.genfromtxt("ResolvedConcentrations.csv", skip_header = 1, delimiter=",", unpack=True)
