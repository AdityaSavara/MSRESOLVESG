...
import test_9_Input as G, importlib; importlib.reload(G) #This is needed because we need the __var_list__
...
try:
    ...
    expected_output1 = np.genfromtxt("ScaledConcentrationsExpected_test_9.csv", skip_header = 1, delimiter =",", dtype = 'f8')
    expected_output2 = np.genfromtxt("ScaledConcentrations_relative_uncertainties_test_9.csv", skip_header = 1, delimiter =",", dtype = 'f8')
    ...
    expected_output1 = np.genfromtxt("ScaledConcentrationsExpected_test_9.csv", skip_header = 1, delimiter =",")
    expected_output2 = np.genfromtxt("ScaledConcentrations_relative_uncertainties_test_9_noUncertaintiesModule.csv", skip_header = 1, delimiter =",")
